//! Implementation of a feature that performs type conversion during forward and backward propagation
...
impl<U,SO,P,I,PI,CI,D> PreTrainBase for BridgeLayer<U,SO,P,I,PI,CI,D>
    where P: PreTrainBase<PreOutput=PI> + PreTrain + ForwardAll<Input=I,Output=PI> +
          ...
          I: Debug + Send + Sync {
    type PreOutput = CI;
    type OutStack = Cons<<P as PreTrainBase>::OutStack, CI>;
}
...
impl<U,SO,P,I,PI,CI,D> BackwardBase for BridgeLayer<U,SO,P,I,PI,CI,D>
    where P: PreTrainBase<PreOutput=PI> + PreTrain + ForwardAll<Input=I,Output=PI> +
             BackwardAll<U,LossInput=PI,LossInputScalar=U> +
             InputTensorScalar + OutputTensorScalar,
...
impl<U,SO,P,I,PI,CI,D> BackwardAll<SO> for BridgeLayer<U,SO,P,I,PI,CI,D>
    where P: PreTrainBase<PreOutput=PI> + PreTrain + ForwardAll<Input=I,Output=PI> +
          ...
          D: Device<U> + DeviceBridge<U,SO,PI,CI>,
          PI: Debug + 'static + BatchDataType + InputTensorScalar,
          CI: Debug + 'static + BatchDataType + OutputTensorScalar<Scalar=SO>,
          I: Debug + Send + Sync {
...
impl<U,SO,P,I,PI,CI,D> BatchBackward<SO> for BridgeLayer<U,SO,P,I,PI,CI,D>
    where P: PreTrainBase<PreOutput=PI> + PreTrain + ForwardAll<Input=I,Output=PI> +
          ...
          D: Device<U> + DeviceBridge<U,SO,PI,CI>,
          PI: Debug + BatchDataType + InputTensorScalar + 'static,
          CI: Debug + BatchDataType + OutputTensorScalar<Scalar=SO> + 'static,
          <PI as BatchDataType>::Type: Debug,
          <CI as BatchDataType>::Type: Debug,
          <I as BatchDataType>::Type: Debug,
          for<'a> <CI as BatchDataType>::Type: Debug,
...
impl<U,SO,P,I,PI,CI,D> Bridge for BridgeLayer<U,SO,P,I,PI,CI,D>
    where P: ForwardAll<Input=I,Output=PI> + BackwardAll<U,LossInput=PI,LossInputScalar=U> +
             PreTrainBase<PreOutput=PI> + PreTrain +
             InputTensorScalar + OutputTensorScalar,
          U: Default + Clone + Copy + Debug + Send + Sync + 'static,
...
impl<U,SO,P,I,PI,CI,D> BridgeRepr for BridgeLayer<U,SO,P,I,PI,CI,D>
    where P: ForwardAll<Input=I,Output=PI> + BackwardAll<U,LossInput=PI,LossInputScalar=U> +
             PreTrainBase<PreOutput=PI> + PreTrain +
             InputTensorScalar + OutputTensorScalar,
          U: Default + Clone + Copy + Debug + Send + Sync + 'static,
...
impl<U,SO,P,I,PI,CI,D> BatchBridge for BridgeLayer<U,SO,P,I,PI,CI,D>
    where P: ForwardAll<Input=I,Output=PI> + BackwardAll<U,LossInput=PI,LossInputScalar=U> +
             PreTrainBase<PreOutput=PI> + PreTrain +
             InputTensorScalar + OutputTensorScalar +
             BatchForwardBase<BatchInput=<I as BatchDataType>::Type,BatchOutput=<PI as BatchDataType>::Type> +
...
impl<U,SO,P,I,PI,CI,D> BatchBridgeRepr for BridgeLayer<U,SO,P,I,PI,CI,D>
    where P: ForwardAll<Input=I,Output=PI> + BackwardAll<U,LossInput=PI,LossInputScalar=U> +
             PreTrainBase<PreOutput=PI> + PreTrain +
             InputTensorScalar + OutputTensorScalar +
             BatchForwardBase<BatchInput=<I as BatchDataType>::Type,BatchOutput=<PI as BatchDataType>::Type> +
...
pub trait BridgeLayerInstantiation<U,SO,P,I,PI,CI,D>
    ...
    /// Instantiate a layer that converts between the Scalar types within the input and output.
    /// # Arguments
    /// * `parent` - upper layer
...
impl<SO,CI> BridgeLayerBuilder<SO,CI>
    ...
    pub fn build<U,P,I,PI,D>(&self,parent:P,device:&D) -> Result<BridgeLayer<U,SO,P,I,PI,CI,D>,LayerInstantiationError>
        where P: ForwardAll<Input=I,Output=PI> +
                 BackwardAll<U,LossInput=PI,LossInputScalar=U> + PreTrainBase<PreOutput=PI> + PreTrain + InputTensorScalar + OutputTensorScalar,
              U: Default + Clone + Copy + Debug + Send + Sync + 'static,
              SO: Default + Clone + Copy + Debug + Send + Sync + 'static,
              D: Device<U>,
