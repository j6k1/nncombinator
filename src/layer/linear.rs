...
use crate::layer::{Backward, BackwardAll, BatchBackward, BatchDataType, BatchForward, BatchForwardBase, BatchPreTrain, BatchPreTrainBase, BatchSize, ContinueForward, Forward, ForwardAll, ForwardDiff, PartialForward, PreTrain, UpdateWeight, OnStep, PersistProgress, InputTensorScalar, OutputTensorScalar, TensorSize, OutputTensorSize, InputTensorSize, InputScale, Bridge, BatchBridge, InputMax, PreTrainBase, BackwardBase, BatchBackwardBase, BridgeRepr, BridgeBase, BatchBridgeRepr};
...
impl<U,C,BC,P,D,I,PI,OP,const NI:usize,const NO:usize> PreTrainBase for LinearLayer<U,C,BC,P,D,I,PI,OP,NI,NO>
...
impl<U,C,BC,P,D,I,PI,OP,const NI:usize,const NO:usize> BatchBackwardBase for LinearLayer<U,C,BC,P,D,I,PI,OP,NI,NO>
    where P: ForwardAll<Input=I,Output=PI> + BackwardAll<U,LossInput=PI,LossInputScalar=U> +
             PreTrainBase<PreOutput=PI> + PreTrain + InputTensorScalar + OutputTensorScalar + BatchForwardBase<BatchInput=<I as BatchDataType>::Type,BatchOutput=<PI as BatchDataType>::Type> + BatchForward +
             BatchPreTrainBase<BatchPreOutput=<PI as BatchDataType>::Type> +
             BatchPreTrain +
...
pub struct QuantizedLinearLayer<U,W,C,BC,P,D,I,PI,LI,OP,const NI:usize,const NO:usize>
    where P: ForwardAll<Input=I,Output=PI> +
             PreTrainBase<PreOutput=PI> + PreTrain +
             InputTensorScalar + OutputTensorScalar + InputMax<Scalar=f32>,
          U: Default + Clone + Copy + Debug + Send + Sync + 'static,
          ...
          [();NO]: TensorSize
{
    ...
    pub fn new<UI, BI, B>(parent: P, device: &D, mut ui: UI, mut bi: BI, b: &B)
        ...
        let (scale,shift,input_scale,max_input,qunits,qbias) = {
            device.quantization(&units, &bias, parent.input_max())?
        };
...
impl<U,W,C,BC,P,D,I,PI,LI,OP,const NI:usize,const NO:usize> Persistence<TextFilePersistence,Specialized>
    ...
    where P: ForwardAll<Input=I,Output=PI> +
          ...
          LI: Debug + InputTensorScalar + OutputTensorScalar + BatchDataType,
          D: Device<U> + Device<f32> + DeviceQuantizedLinear<U,W,C,BC,PI,NI,NO>,
          OP: Optimizer<f32,D>,
          TextRecord: From<f32>,
    ...
    fn load(&mut self, persistence: &mut TextFilePersistence) -> Result<(), ModelLoadError> {
        ...
        let (scale,shift,input_scale,max_input,qunits,qbias) = {
            self.device.quantization(&units, &bias, self.parent.input_max())?
        };
...
impl<T,U,W,C,BC,P,D,I,PI,LI,OP,const NI:usize,const NO:usize> Persistence<T,Linear>
    for QuantizedLinearLayer<U,W,C,BC,P,D,I,PI,LI,OP,NI,NO>
    where T: LinearPersistence<f32>,
          P: ForwardAll<Input=I,Output=PI> +
             PreTrainBase<PreOutput=PI> + PreTrain + InputScale +
             InputTensorScalar + OutputTensorScalar + InputMax<Scalar=f32> + Persistence<T,Linear>,
...
impl<U,W,C,BC,P,D,I,PI,LI,OP,const NI:usize,const NO:usize> Forward<PI,Result<<Self as ForwardAll>::Output,EvaluateError>>
    for QuantizedLinearLayer<U,W,C,BC,P,D,I,PI,LI,OP,NI,NO>
    where P: ForwardAll<Input=I,Output=PI> +
             PreTrainBase<PreOutput=PI> + PreTrain +
             InputTensorScalar + OutputTensorScalar + InputMax<Scalar=f32>,
...
impl<U,W,C,BC,P,D,I,PI,LI,OP,const NI:usize,const NO:usize> ForwardAll
    for QuantizedLinearLayer<U,W,C,BC,P,D,I,PI,LI,OP,NI,NO>
    where P: ForwardAll<Input=I,Output=PI> +
             PreTrainBase<PreOutput=PI> + PreTrain +
             InputTensorScalar + OutputTensorScalar + InputMax<Scalar=f32>,
...
impl<U,W,C,BC,P,D,I,PI,LI,OP,const NI:usize,const NO:usize> PreTrainBase
    for QuantizedLinearLayer<U,W,C,BC,P,D,I,PI,LI,OP,NI,NO>
    where P: PreTrainBase<PreOutput=PI> + PreTrain +
...
impl<U,W,C,BC,P,D,I,PI,LI,OP,const NI:usize,const NO:usize> PreTrain
    for QuantizedLinearLayer<U,W,C,BC,P,D,I,PI,LI,OP,NI,NO>
    where P: PreTrainBase<PreOutput=PI> + PreTrain +
             ForwardAll<Input=I,Output=PI> +
             InputTensorScalar + OutputTensorScalar + InputMax<Scalar=f32>,
...
impl<U,W,C,BC,P,D,I,PI,LI,OP,const NI:usize,const NO:usize>
    Backward<f32,&<D as DeviceQuantizedLinear<U,W,C,BC,PI,NI,NO>>::LossInput,Result<<D as DeviceQuantizedLinear<U,W,C,BC,PI,NI,NO>>::LossOutput,TrainingError>>
    for QuantizedLinearLayer<U,W,C,BC,P,D,I,PI,LI,OP,NI,NO>
    where P: ForwardAll<Input=I,Output=PI> + BackwardAll<f32,LossInput=<D as DeviceQuantizedLinear<U,W,C,BC,PI,NI,NO>>::LossOutput,LossInputScalar=f32> +
          ...
          [();NI]: TensorSize,
          [();NO]: TensorSize {
    fn backward(&mut self, input: &<D as DeviceQuantizedLinear<U,W,C,BC,PI,NI,NO>>::LossInput)
        -> Result<<D as DeviceQuantizedLinear<U,W,C,BC,PI,NI,NO>>::LossOutput,TrainingError> {
        Ok(self.device.backward_linear(&self.units,input)?.into())
    }
}
impl<U,W,C,BC,P,D,I,PI,LI,OP,const NI:usize,const NO:usize> BackwardBase
    for QuantizedLinearLayer<U,W,C,BC,P,D,I,PI,LI,OP,NI,NO>
    where P: BackwardBase<LossInput=<D as DeviceQuantizedLinear<U,W,C,BC,PI,NI,NO>>::LossOutput,LossInputScalar=f32> +
             BackwardAll<f32> +
             ForwardAll<Input=I,Output=PI> +
             PreTrainBase<PreOutput=PI> + PreTrain +
             ...
             InputTensorScalar + OutputTensorScalar + InputScale + InputMax<Scalar=f32>,
          ...
          LI: Debug + BatchDataType +
              InputTensorScalar<Scalar=f32> + OutputTensorScalar<Scalar=f32>,
...
impl<U,W,C,BC,P,D,I,PI,LI,OP,const NI:usize,const NO:usize> BackwardAll<f32>
    for QuantizedLinearLayer<U,W,C,BC,P,D,I,PI,LI,OP,NI,NO>
    where P: BackwardBase<LossInput=<D as DeviceQuantizedLinear<U,W,C,BC,PI,NI,NO>>::LossOutput,LossInputScalar=f32> +
             BackwardAll<f32> +
             ForwardAll<Input=I,Output=PI> +
             PreTrainBase<PreOutput=PI> + PreTrain +
             BridgeBase<SourceInput=PI> +
             BridgeBase<RealScale=<D as DeviceQuantizedLinear<U,W,C,BC,PI,NI,NO>>::LossOutput> +
             BridgeBase<RealOutput=<D as DeviceQuantizedLinear<U,W,C,BC,PI,NI,NO>>::LossOutput> + Bridge +
             InputTensorScalar + OutputTensorScalar + InputScale + InputMax<Scalar=f32>,
          U: Default + Clone + Copy + Debug + Send + Sync + MaxValue + Assume<f32> + Mul<Output=U> + 'static,
          W: Default + Clone + Copy + Debug + Send + Sync + MaxValue + Assume<U> + 'static,
          I: Debug + Send + Sync,
          ...
          PI: Debug + InputTensorSize<NI> + BatchDataType +
              InputTensorScalar<Scalar=U> + OutputTensorScalar<Scalar=U> + 'static,
          LI: Debug + BatchDataType + InputTensorScalar<Scalar=f32> + OutputTensorScalar<Scalar=f32>,
          D: Device<U> + Device<f32> + DeviceQuantizedLinear<U,W,C,BC,PI,NI,NO,LossInput=LI> +
             DeviceBatchAveraging<C,f32> + DeviceBatchAveraging<BC,f32>,
          C: Quantizable<W>,
          BC: Quantizable<W>,
          ...
          for<'a> <OP as Optimizer<f32,D>>::InternalUpdateType<'a>: From<&'a mut C>,
          for<'a> <OP as Optimizer<f32,D>>::InternalUpdateType<'a>: From<&'a mut BC>,
          for<'a> <P as Bridge>::RealMapper<'a>: Deref<Target=<D as DeviceQuantizedLinear<U,W,C,BC,PI,NI,NO>>::RealInput>,
          [();NI]: TensorSize,
          [();NO]: TensorSize {
    ...
    fn backward_all(&mut self, input: Self::LossInput, stack:Self::OutStack)
        ...
        let g = s.map(|o| {
            self.parent.as_real(o).map(|o| {
                self.device.backward_weight_gradient(o.deref(),&loss)
            })
        })??;

        let bg = self.device.backward_bias_weight_gradient(loss)?;

        let (l,s) = self.parent.backward_all(next_loss, s)?;

        Ok((l,Cons(s,(g,bg))))
    }
}
impl<U,W,C,BC,P,D,I,PI,LI,OP,const NI:usize,const NO:usize> PartialForward
    for QuantizedLinearLayer<U,W,C,BC,P,D,I,PI,LI,OP,NI,NO>
    where P: ForwardAll<Input=I,Output=PI> + PartialForward +
             PreTrainBase<PreOutput=PI> + PreTrain +
             InputTensorScalar + OutputTensorScalar + InputMax<Scalar=f32>,
...
impl<U,W,C,BC,P,D,I,PI,LI,OP,const NI:usize,const NO:usize> ForwardDiff
    for QuantizedLinearLayer<U,W,C,BC,P,D,I,PI,LI,OP,NI,NO>
    where P: ForwardAll<Input=I,Output=PI> + PartialForward + ForwardDiff +
             PreTrainBase<PreOutput=PI> + PreTrain +
             InputTensorScalar + OutputTensorScalar + InputMax<Scalar=f32>,
...
impl<U,W,C,BC,P,D,I,PI,LI,OP,const NI:usize,const NO:usize> ContinueForward
    for QuantizedLinearLayer<U,W,C,BC,P,D,I,PI,LI,OP,NI,NO>
    where P: ForwardAll<Input=I,Output=PI> + PartialForward + ContinueForward +
             PreTrainBase<PreOutput=PI> + PreTrain +
             InputTensorScalar + OutputTensorScalar + InputMax<Scalar=f32>,
...
impl<U,W,C,BC,P,D,I,PI,LI,OP,const NI:usize,const NO:usize> UpdateWeight
    for QuantizedLinearLayer<U,W,C,BC,P,D,I,PI,LI,OP,NI,NO>
    where P: ForwardAll<Input=I,Output=PI> +
    ...
    fn update_weight(&mut self, stack: Self::GradientStack, batch_size: usize) -> Result<(), TrainingError> {
        ...
        let (scale,shift,input_scale,max_input,qunits,qbias) = {
            self.device.quantization(&self.units, &self.bias, self.parent.input_max())?
        };
...
impl<U,W,C,BC,P,D,I,PI,LI,OP,const NI:usize,const NO:usize> BatchForwardBase
    for QuantizedLinearLayer<U,W,C,BC,P,D,I,PI,LI,OP,NI,NO>
    where P: ForwardAll<Input=I,Output=PI> +
             PreTrainBase<PreOutput=PI> + PreTrain +
             InputTensorScalar + OutputTensorScalar + InputMax<Scalar=f32> +
...
impl<U,W,C,BC,P,OP,D,I,PI,LI,const NI:usize,const NO:usize> BatchForward
    for QuantizedLinearLayer<U,W,C,BC,P,D,I,PI,LI,OP,NI,NO>
    where P: ForwardAll<Input=I,Output=PI> +
             PreTrainBase<PreOutput=PI> + PreTrain +
             Bridge<RealScale=<D as DeviceQuantizedLinear<U,W,C,BC,PI,NI,NO>>::LossOutput> +
...
impl<U,W,C,BC,P,D,I,PI,LI,OP,const NI:usize,const NO:usize> BatchPreTrainBase
    for QuantizedLinearLayer<U,W,C,BC,P,D,I,PI,LI,OP,NI,NO>
    where P: ForwardAll<Input=I,Output=PI> +
             PreTrainBase<PreOutput=PI> + PreTrain +
             InputTensorScalar + OutputTensorScalar + InputMax<Scalar=f32> +
          ...
          BC: Quantizable<W>,
          PI: Debug + InputTensorSize<NI> + BatchDataType +
              InputTensorScalar<Scalar=U> + OutputTensorScalar<Scalar=U> + BatchDataType,
          LI: Debug + BatchDataType + InputTensorScalar + OutputTensorScalar,
          <I as BatchDataType>::Type: Debug,
...
impl<U,W,C,BC,P,D,I,PI,LI,OP,const NI:usize,const NO:usize> BatchPreTrain
    for QuantizedLinearLayer<U,W,C,BC,P,D,I,PI,LI,OP,NI,NO>
    where P: ForwardAll<Input=I,Output=PI> +
             PreTrainBase<PreOutput=PI> + PreTrain +
             Bridge<RealScale=<D as DeviceQuantizedLinear<U,W,C,BC,PI,NI,NO>>::LossOutput> +
...
impl<U,W,C,BC,P,D,I,PI,LI,OP,const NI:usize,const NO:usize> BatchBackwardBase
    for QuantizedLinearLayer<U,W,C,BC,P,D,I,PI,LI,OP,NI,NO>
    where P: ForwardAll<Input=I,Output=PI> +
             BackwardBase<LossInput=<D as DeviceQuantizedLinear<U,W,C,BC,PI,NI,NO>>::LossOutput,LossInputScalar=f32> +
             BackwardAll<f32> +
             Bridge<RealScale=<D as DeviceQuantizedLinear<U,W,C,BC,PI,NI,NO>>::LossOutput> +
             Bridge<RealOutput=<D as DeviceQuantizedLinear<U,W,C,BC,PI,NI,NO>>::LossOutput> +
             PreTrainBase<PreOutput=PI> + PreTrain + InputTensorScalar + OutputTensorScalar + InputScale + InputMax<Scalar=f32> +
             BatchForwardBase<BatchInput=<I as BatchDataType>::Type,BatchOutput=<PI as BatchDataType>::Type> +
             BatchForward +
             BatchPreTrainBase<BatchPreOutput=<PI as BatchDataType>::Type> +
             BatchPreTrain +
             BatchBackward<f32,BatchLossInput=<D as DeviceQuantizedLinear<U,W,C,BC,PI,NI,NO>>::BatchLossOutput>,
          U: Default + Clone + Copy + Debug + Mul<Output=U> + MaxValue + Assume<f32> + Send + Sync + 'static,
          W: Default + Clone + Copy + Debug + Send + Sync + MaxValue + Assume<U> + 'static,
          I: Debug + Send + Sync + BatchDataType,
          C: Debug,
          BC: Debug,
          PI: BatchDataType + InputTensorSize<NI> + InputTensorScalar<Scalar=U> + OutputTensorScalar<Scalar=U> + Debug + 'static,
          LI: BatchDataType + Debug + InputTensorScalar<Scalar=f32> + OutputTensorScalar<Scalar=f32>,
          OP: Optimizer<f32,D>,
          <I as BatchDataType>::Type: Debug,
          ...
          for<'a> <OP as Optimizer<f32,D>>::InternalUpdateType<'a>: From<&'a mut C>,
          for<'a> <OP as Optimizer<f32,D>>::InternalUpdateType<'a>: From<&'a mut BC> {
    type BatchLossInput = <D as DeviceQuantizedLinear<U,W,C,BC,PI,NI,NO>>::BatchLossInput;
    type BatchLossOutput = <P as BatchBackwardBase>::BatchLossOutput;
}
impl<U,W,C,BC,P,D,I,PI,LI,OP,const NI:usize,const NO:usize> BatchBackward<f32>
    for QuantizedLinearLayer<U,W,C,BC,P,D,I,PI,LI,OP,NI,NO>
    where P: ForwardAll<Input=I,Output=PI> + BackwardAll<f32,LossInput=<D as DeviceQuantizedLinear<U,W,C,BC,PI,NI,NO>>::LossOutput,LossInputScalar=f32> +
             PreTrainBase<PreOutput=PI> + PreTrain + InputTensorScalar + OutputTensorScalar + InputScale + InputMax<Scalar=f32> +
             BridgeBase<SourceInput=PI> +
             BridgeBase<RealScale=<D as DeviceQuantizedLinear<U,W,C,BC,PI,NI,NO>>::LossOutput> +
             BridgeBase<RealOutput=<D as DeviceQuantizedLinear<U,W,C,BC,PI,NI,NO>>::LossOutput> +
             Bridge + BatchBridge +
             BatchForwardBase<BatchInput=<I as BatchDataType>::Type,BatchOutput=<PI as BatchDataType>::Type> +
             BatchForward +
             BatchPreTrainBase<BatchPreOutput=<PI as BatchDataType>::Type> +
    ...
    fn batch_backward(&mut self, input: Self::BatchLossInput, stack: Self::BatchOutStack)
        ...
        let g = s.map(|o| {
            self.parent.batch_as_real(o).map(|o| {
                self.device.batch_backward_weight_gradient(o.deref(), &loss)
            })
        })??;

        let bg = self.device.batch_backward_bias_gradient(&loss)?;
...
impl<U,W,C,BC,P,D,I,PI,LI,OP,const NI:usize,const NO:usize> OnStep
    ...
    where P: ForwardAll<Input=I,Output=PI> +
             PreTrainBase<PreOutput=PI> + PreTrain +
             InputTensorScalar + OutputTensorScalar + InputMax<Scalar=f32> + OnStep,
          U: Default + Clone + Copy + Debug + Send + Sync + 'static,
          W: Default + Clone + Copy + Debug + Send + Sync + 'static,
          D: Device<U> + Device<f32> + DeviceQuantizedLinear<U,W,C,BC,PI,NI,NO>,
          I: Debug + Send + Sync,
...
impl<T,U,W,C,BC,P,D,I,PI,LI,OP,const NI:usize,const NO:usize> PersistProgress<T,Linear>
    for QuantizedLinearLayer<U,W,C,BC,P,D,I,PI,LI,OP,NI,NO>
    where T: LinearPersistence<U>,
          P: ForwardAll<Input=I,Output=PI> +
             PreTrainBase<PreOutput=PI> + PreTrain +
...
impl<U,W,C,BC,P,D,I,PI,LI,OP,const NI:usize,const NO:usize> InputScale
    for QuantizedLinearLayer<U,W,C,BC,P,D,I,PI,LI,OP,NI,NO>
    where P: ForwardAll<Input=I,Output=PI> +
             PreTrainBase<PreOutput=PI> + PreTrain +
             InputTensorScalar + OutputTensorScalar + InputMax<Scalar=f32>,
...
impl<U,W,C,BC,P,D,I,PI,LI,OP,const NI:usize,const NO:usize> Bridge
    ...
    where P: ForwardAll<Input=I,Output=PI> +
          ...
          LI: Debug + Sized + BatchDataType + InputTensorScalar + OutputTensorScalar + 'static,
          D: Device<U> + DeviceQuantizedLinear<U,W,C,BC,PI,NI,NO,LossInput=LI,Scale=LI> +
             DeviceBridge<U,f32,<D as DeviceQuantizedLinear<U,W,C,BC,PI,NI,NO>>::Output,LI> + DeviceScale<f32,LI,NO>,
          <I as BatchDataType>::Type: Debug,
          <PI as BatchDataType>::Type: BatchSize + IntoConverter + Debug + 'static,
          <LI as BatchDataType>::Type: Debug + BatchSize + 'static,
          <D as DeviceQuantizedLinear<U,W,C,BC,PI,NI,NO>>::LossOutput: BatchDataType + Debug,
...
impl<U,W,C,BC,P,D,I,PI,LI,OP,const NI:usize,const NO:usize> BatchBridge
    ...
    where P: ForwardAll<Input=I,Output=PI> +
             ...
             InputTensorScalar + OutputTensorScalar + InputMax<Scalar=f32> + 'static,
          U: Default + Clone + Copy + Debug + Send + Sync + Assume<f32> + 'static,
          W: Default + Clone + Copy + Debug + Send + Sync + 'static,
...
impl<U,W,C,BC,P,D,I,PI,LI,OP,const NI:usize,const NO:usize> InputMax
    for QuantizedLinearLayer<U,W,C,BC,P,D,I,PI,LI,OP,NI,NO>
    where P: ForwardAll<Input=I,Output=PI> +
             PreTrainBase<PreOutput=PI> + PreTrain +
             InputTensorScalar + OutputTensorScalar + InputMax<Scalar=f32>,
...
pub trait QuantizedLinearLayerInstantiation<U,W,C,BC,P,D,I,PI,LI,OP,const NI:usize,const NO:usize>
    where P: ForwardAll<Input=I,Output=PI> +
             PreTrainBase<PreOutput=PI> + PreTrain +
             InputTensorScalar + OutputTensorScalar + InputMax<Scalar=f32>,
          U: Default + Clone + Copy + Debug + Send + Sync + 'static,
...
impl<U,W,C,BC,P,D,I,PI,LI,OP,const NI:usize,const NO:usize> QuantizedLinearLayerInstantiation<U,W,C,BC,P,D,I,PI,LI,OP,NI,NO>
    for QuantizedLinearLayer<U,W,C,BC,P,D,I,PI,LI,OP,NI,NO>
    where P: ForwardAll<Input=I,Output=PI> +
             PreTrainBase<PreOutput=PI> + PreTrain + InputScale +
             InputTensorScalar + OutputTensorScalar + InputMax<Scalar=f32>,
    ...
    fn instantiation<B: OptimizerBuilder<f32,D,Output=OP>>(parent: P, device:&D, ui: impl FnMut() -> f32, bi: impl FnMut() -> f32, b: &B)
        -> Result<QuantizedLinearLayer<U,W,C,BC,P,D,I,PI,LI,OP,NI,NO>,LayerInstantiationError> {
        Ok(QuantizedLinearLayer::<_,_,_,_,_,_,_,_,_,_,NI,NO>::new(parent,device,ui,bi,b)?)
    }
}
/// Builder for QuantizedLinearLayer instance creation
pub struct QuantizedLinearLayerBuilder<W,U,const NI:usize,const NO:usize>
    where [();NI]: TensorSize,
          [();NO]: TensorSize {
    w:PhantomData<W>,
    u:PhantomData<U>,
...
impl<W,U,const NI:usize,const NO:usize> QuantizedLinearLayerBuilder<W,U,NI,NO>
    where[();NI]: TensorSize,
         [();NO]: TensorSize {
    /// Create an instance of QuantizedLinearLayerBuilder
    pub fn new() -> QuantizedLinearLayerBuilder<W,U,NI,NO> {
...
impl<W,U,const NI:usize,const NO:usize> QuantizedLinearLayerBuilder<W,U,NI,NO>
    ...
    /// This function may return the following errors
    /// * [`LayerInstantiationError`]
    pub fn build<C,BC,P,D,I,PI,LI,OP,B>(&self,parent: P, device:&D, ui: impl FnMut() -> f32, bi: impl FnMut() -> f32, b: &B)
        -> Result<QuantizedLinearLayer<U,W,C,BC,P,D,I,PI,LI,OP,NI,NO>,LayerInstantiationError>
        where P: ForwardAll<Input=I,Output=PI> +
                 PreTrainBase<PreOutput=PI> + PreTrain +
                 InputTensorScalar + OutputTensorScalar + InputMax<Scalar=f32>,
              ...
              [();NI]: TensorSize,
              [();NO]: TensorSize,
              QuantizedLinearLayer<U,W,C,BC,P,D,I,PI,LI,OP,NI,NO>: QuantizedLinearLayerInstantiation<U,W,C,BC,P,D,I,PI,LI,OP,NI,NO> {
...
impl<'a,U,C,BC,P,OP,D,I,DI,PI,const NI:usize,const NO:usize> PreTrainBase for DiffLinearLayer<'a,U,C,BC,P,OP,D,I,DI,PI,NI,NO>
    where P: PreTrainBase<PreOutput=PI> + PreTrain +
             ForwardAll<Input=I,Output=PI> +
             BackwardAll<U,LossInput=()> +
             InputTensorScalar + OutputTensorScalar,
