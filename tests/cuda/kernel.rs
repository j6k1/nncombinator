...
fn test_kernel_forward_linear_batch_matches_cpu() {
    ...
    let d_output = CudaVec::<f64,CudaTensor1dPtr<f64,A,NO>,A>::new(BATCH,alloc).unwrap();

    // Build args
    let d_inputs = &d_inputs;
    let conv = d_inputs.as_cuda_view().into_converter();
    let input_view = CudaVecView::<f64,CudaTensor1dPtrView<f64,NI>>::try_from(conv).unwrap();
    let mut args = ForwardLinearBatchArgs::<'_,f64,A,NI,NO>::new(&input_view,&d_units,&d_bias,d_output,BATCH);

    let mut kernel = ForwardLinearBatch::<'_,f64,A,NI,NO>::new();
...
fn test_kernel_backward_linear_batch_matches_cpu() {
    ...
    let d_output = CudaVec::<f64,CudaTensor1dPtr<f64,A,NI>,A>::new(BATCH,alloc).unwrap();

    // Build args
    let loss_view = CudaVecView::<f64,CudaTensor1dPtrView<f64,NO>>::try_from(&d_loss).unwrap();
    let mut args = BackwardLinearBatchArgs::<'_,f64,A,NI,NO>::new(&loss_view,&d_units,d_output,BATCH);

    let mut kernel = BackwardLinearBatch::<'_,f64,A,NI,NO>::new();
    kernel.launch(&mut args).unwrap();
...
fn test_kernel_linear_gradient_batch_matches_cpu() {
    ...
    // Output buffer (zero-init required)
    let d_output = CudaTensor2dPtr::<f64,A,NI,NO>::with_initializer(alloc,|| 0.0f64).unwrap();

    // Build args
    let d_inputs = &d_inputs;
    let input_view_conv = d_inputs.as_cuda_view().into_converter();
    let input_view = CudaVecView::<f64,CudaTensor1dPtrView<f64,NI>>::try_from(input_view_conv).unwrap();
    let loss_view = CudaVecView::<f64,CudaTensor1dPtrView<f64,NO>>::try_from(&d_loss).unwrap();
    let mut args = LinearGradientBatchArgs::<'_,f64,A,NI,NO>::new(&loss_view,&input_view,d_output,BATCH);

    let mut kernel = LinearGradientBatch::<'_,f64,A,NI,NO>::new();
    kernel.launch(&mut args).unwrap();
...
fn test_kernel_reduce_linear_batch_matches_cpu() {
    ...
    let input = SerializedVec::<f64,Arr<f64,1000>>::from(input);

    let cpu_out:Arr<f64,1000> = device.reduce(&input).unwrap();

    let input = &input;

    let allocator:&A = &SHARED_MEMORY_POOL;

    let mut input_ptr = CudaVec::<f64,CudaTensor1dPtr<f64,A,1000>,A>::new(input.size(),allocator).unwrap();
    ...
    let output_ptr = CudaTensor1dPtr::<f64,A,1000>::new(allocator).unwrap();

    let mut args = ReduceLinearBatchArgs::new(&input_ptr,output_ptr,1000,input.size());

    let mut kernel = ReduceLinearBatch::<f64,A,1000>::new();

    kernel.launch(&mut args).unwrap();

    let gpu_out = args.output.read_to_vec().unwrap();

    let mut cpu_flat: Vec<f64> = Vec::with_capacity(1200 * 1000);
